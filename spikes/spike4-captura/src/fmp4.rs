...
impl FluxoDeSaida {
    ...
    /// Escreve na posição atual e empurra para fora o que ficou para lá da marca de
    /// entrega. Escrever por cima de bytes já entregues não os reenvia — e não acontece
    /// com MP4 fragmentado a não ser nas correções de tamanho de caixa.
    fn escrever(&self, bytes: &[u8]) -> u32 {
        ...
        let pedaco = if fim > e.entregue {
            let p = e.dados[e.entregue..fim].to_vec();
            e.entregue = fim;
            Some(p)
        } else {
            None
